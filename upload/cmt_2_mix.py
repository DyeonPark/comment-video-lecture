...
def mix(tts_path, lec_path, save_path, default_path, mix_path):
    ...
    combined = AudioSegment.from_mp3(default_path + "lec/lec_0001.mp3")

    #File = 'transform_timeline_result.csv'
    ...
    for i,j in zip(range(len(time_csv["slide"])),data_list):
        tts_list[i] = AudioSegment.from_mp3(default_path + "tts/" + "tts_" + j + ".mp3")
...
def execute_mix(default_path):
    ...
    pdf_path = default_path + "lecture_doc_all.pdf"
    ...
    # 모든 오디오 파일 병합 - mix
    tmp_start = time.time()
    mix(tts_path, lec_path, save_path, default_path, mix_path)
